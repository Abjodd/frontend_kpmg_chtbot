...
// ── Hardcoded credentials — swap these for DB/API calls later ──────────────
const User_CREDENTIALS: Record<string, string> = {
  "User":       "User123",
  "john.doe":    "kpmg@2024",
  "sarah.smith": "Password1",
};
// ───────────────────────────────────────────────────────────────────────────
...
export default function UserLogin() {
  ...
  const handleLogin = () => {
    ...
    // TODO: replace this block with a real fetch/DB call
    setTimeout(() => {
      const expected = User_CREDENTIALS[username.trim().toLowerCase()];
      if (expected && expected === password) {
        localStorage.setItem("user", JSON.stringify({ name: username.trim(), role: "User" }));
        router.push("/chat");
      } else {
  ...
  return (
    <>
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .scene {
          ...
          background:radial-gradient(ellipse at 70% 50%, #00112e 0%, #000 65%);
        }
        .grid-floor {
          position:absolute; inset:0;
          background-image:linear-gradient(rgba(0,91,184,0.18) 1px,transparent 1px),linear-gradient(90deg,rgba(0,91,184,0.18) 1px,transparent 1px);
        ...
        @keyframes floatUp {
          ...
          10%  { opacity:1; }
          90%  { opacity:.5; }
          100% { transform:translateY(-10vh) translateX(var(--dx,0));opacity:0; }
        }
        ...
        .card-inner {
          position:relative;background:rgba(0,4,18,.88);border:1px solid rgba(0,145,218,.2);border-radius:4px;
          padding:44px 44px 40px;backdrop-filter:blur(28px);
          box-shadow:0 0 0 1px rgba(0,51,141,.12),0 40px 80px rgba(0,0,0,.85),inset 0 1px 0 rgba(255,255,255,.05);
          overflow:hidden;
        }
        ...
        .User-badge { display:inline-flex;align-items:center;gap:7px;background:rgba(109,32,119,.2);border:1px solid rgba(109,32,119,.4);border-radius:2px;padding:4px 12px;margin-bottom:16px;animation:fadeDown .7s .45s both; }
        .User-badge-dot { width:5px;height:5px;border-radius:50%;background:#a855f7;box-shadow:0 0 8px #a855f7; }
        .User-badge-text { font-size:9px;letter-spacing:3px;text-transform:uppercase;color:#c084fc; }
        ...
        .error-box {
          ...
          background:rgba(229,62,62,.1);border:1px solid rgba(229,62,62,.3);border-radius:3px;
        ...
        .locked-banner {
          ...
          background:rgba(229,62,62,.1);border:1px solid rgba(229,62,62,.35);border-radius:3px;
        ...
        .input-field {
          width:100%;background:rgba(255,255,255,.04);border:1px solid rgba(0,145,218,.2);border-radius:3px;
          padding:13px 42px 13px 16px;font-family:'DM Sans',sans-serif;font-size:14px;font-weight:400;
          color:#fff;letter-spacing:.4px;outline:none;caret-color:var(--light);
        ...
        .btn {
          position:relative;width:100%;padding:15px 20px;border-radius:3px;border:none;cursor:pointer;
          font-family:'DM Sans',sans-serif;font-size:11px;font-weight:500;letter-spacing:3px;text-transform:uppercase;
          color:#fff;background:var(--blue);overflow:hidden;
          transition:transform .2s,box-shadow .2s,opacity .2s;
          box-shadow:0 0 30px rgba(0,51,141,.5),inset 0 1px 0 rgba(255,255,255,.1);
        }
      ...
      <div className="scene">
        ...
        <div className="vert-text vert-left">KPMG · User · SECURE</div>
        <div className="vert-text vert-right">ELEVATED ACCESS LEVEL</div>
        <div className="badge badge-tl">USER-PORTAL</div>
        ...
        <div className="card">
          <div className="card-inner">
            ...
            <div className="User-badge">
              ...
              <span className="User-badge-text">User Access</span>
            </div>

            <div className="divider" />
            <div className="eyebrow">Secure Login</div>
            <div className="title">User<br /><em>Portal</em></div>
            ...
            {locked && (
              <div className="locked-banner">
                ...
                <div className="locked-text">
                  Account locked after {MAX_ATTEMPTS} failed attempts.<br />
                  Contact your system Useristrator to unlock.
                </div>
              </div>
            )}
            ...
            <div className="btn-wrap">
              <button
                className="btn"
                onClick={handleLogin}
                disabled={!username.trim() || !password || loading || locked}
              >
                {loading ? "Authenticating..." : locked ? "Account Locked" : "Login as User"}
            ...
            <div className="status">
              ...
              <div className="status-text">
                Secure channel <strong>established</strong> · User node ready
              </div>
            </div>
          </div>
        </div>
