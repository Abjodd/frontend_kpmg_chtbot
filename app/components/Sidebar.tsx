...
export function createNewChat(chats: any[]) {
  const newChat = {
    id: Date.now(),
...
export function deleteChat(chats: any[], index: number) {
...
export default function Sidebar({ chats, setChats, selectChat, activeChat }: any) {
  ...
  const handleNewChat = () => {
    const updated = createNewChat(chats);
    setChats(updated);
    selectChat(0);
  };
  ...
  const handleDelete = (e: React.MouseEvent, index: number) => {
    ...
    if (confirmDelete === index) {
      const updated = deleteChat(chats, index);
      setChats(updated);
      setConfirmDelete(null);
      selectChat(Math.min(activeChat, updated.length - 1));
    } else {
    ...
    <>
      <style>{`
        ...
        .sb-root {
          ...
          background: rgba(0, 3, 14, 0.97); border-right: 1px solid rgba(0,145,218,0.12);
          position: relative; overflow: hidden; font-family: 'DM Sans', sans-serif;
          flex-shrink: 0; transition: width 0.32s cubic-bezier(0.16,1,0.3,1);
        }
        .sb-root.open   { width: 270px; }
        .sb-root.closed { width: 56px; }

        .sb-grid {
          position:absolute;inset:0;pointer-events:none;z-index:0;
          background-image:linear-gradient(rgba(0,91,184,0.07) 1px,transparent 1px),linear-gradient(90deg,rgba(0,91,184,0.07) 1px,transparent 1px);
        ...
        .sb-toggle {
          ...
          width:100%;padding:0;height:56px;border:none;background:transparent;
          border-bottom:1px solid rgba(0,145,218,0.12);cursor:pointer;flex-shrink:0;
          transition:background .18s;gap:10px;overflow:hidden;
        }
        .sb-toggle:hover { background:rgba(0,145,218,0.05); }

        .sb-logo-box {
          width:32px;height:32px;background:#00338D;border-radius:2px;display:flex;align-items:center;justify-content:center;
          font-family:'Bebas Neue',sans-serif;font-size:11px;color:#fff;letter-spacing:1px;
          box-shadow:0 0 14px rgba(0,51,141,0.6);position:relative;overflow:hidden;flex-shrink:0;transition:box-shadow .2s;
        }
        ...
        .sb-root.open   .sb-header-info { max-height:160px;opacity:1;padding:14px 16px 12px; }
        ...
        .sb-new-btn {
          ...
          padding:5px 10px;border-radius:2px;border:1px solid rgba(0,145,218,0.25);
          ...
          color:rgba(0,145,218,0.7);transition:background .18s,border-color .18s,color .18s,box-shadow .18s;
        }
        ...
        .sb-clear-btn {
          ...
          padding:5px 8px;border-radius:2px;border:1px solid rgba(229,62,62,0.15);
          background:transparent;cursor:pointer;color:rgba(229,62,62,0.35);
        ...
        .sb-root.open   .sb-search-wrap { max-height:60px;opacity:1; }
        .sb-root.closed .sb-search-wrap { max-height:0;opacity:0;padding:0; }

        .sb-search {
          width:100%;background:rgba(255,255,255,0.04);border:1px solid rgba(0,145,218,0.15);border-radius:3px;
          padding:7px 10px 7px 30px;font-family:'DM Sans',sans-serif;font-size:12px;color:#fff;outline:none;
          caret-color:#0091DA;transition:border-color .2s,background .2s;
        }
        .sb-search::placeholder { color:rgba(255,255,255,0.18); }
        .sb-search:focus { border-color:rgba(0,145,218,0.4);background:rgba(0,145,218,0.05); }
        ...
        /* new chat mini (collapsed) */
        .sb-mini-new {
          position:relative;z-index:5;display:flex;align-items:center;justify-content:center;
          padding:8px 0;border-bottom:1px solid rgba(0,145,218,0.1);flex-shrink:0;
          transition:opacity .2s;cursor:pointer;background:transparent;border-left:none;border-right:none;border-top:none;
          width:100%;
        }
        .sb-root.open   .sb-mini-new { opacity:0;pointer-events:none;max-height:0;padding:0;overflow:hidden; }
        .sb-root.closed .sb-mini-new { opacity:1;max-height:48px; }
        .sb-mini-new:hover { background:rgba(0,145,218,0.07); }
        .sb-mini-new svg { color:rgba(0,145,218,0.5); }

        /* scroll */
        .sb-scroll { flex:1;overflow-y:auto;overflow-x:hidden;padding:12px 8px;position:relative;z-index:5; }
        .sb-scroll::-webkit-scrollbar { width:3px; }
        .sb-scroll::-webkit-scrollbar-track { background:transparent; }
        .sb-scroll::-webkit-scrollbar-thumb { background:rgba(0,145,218,0.2);border-radius:2px; }
        ...
        .sb-root.open   .sb-item-right { max-width:60px;opacity:1; }
        .sb-root.closed .sb-item-right { max-width:0;opacity:0; }
        ...
        .sb-item:not(.active) .sb-item-dot { opacity:0; }

        /* delete btn */
        .sb-delete-btn {
          display:none;background:transparent;border:none;cursor:pointer;
          color:rgba(255,255,255,0.2);padding:2px;border-radius:2px;
          transition:color .15s,background .15s;line-height:1;align-items:center;justify-content:center;
        }
        ...
        .sb-root.closed .sb-item:hover::before {
          ...
          background:rgba(0,3,16,0.97);border:1px solid rgba(0,145,218,0.25);border-radius:3px;
          padding:5px 10px;font-size:11px;color:rgba(255,255,255,0.7);white-space:nowrap;pointer-events:none;z-index:100;box-shadow:0 4px 20px rgba(0,0,0,0.5);
        }
        .sb-root.closed .sb-item.active::before {
          left:calc(100% + 10px);top:50%;transform:translateY(-50%);width:auto;height:auto;
          border-radius:3px;background:rgba(0,3,16,0.97);border:1px solid rgba(0,145,218,0.25);
          padding:5px 10px;font-size:11px;color:rgba(255,255,255,0.7);content:attr(data-tip);white-space:nowrap;pointer-events:none;z-index:100;
        }
      ...
      <div className={`sb-root ${open ? "open" : "closed"}`}>
        ...
        <div className="sb-header-info">
          ...
          <div className="sb-header-actions">
            ...
            {chats.length > 0 && (
              <button className="sb-clear-btn" onClick={handleClearAll} title="Clear all history">
        ...
        <div className="sb-scroll">
          ...
          {chats.length > 0 && filtered.length === 0 && (
            <div className="sb-no-results">No matches</div>
          )}

          {filtered.map((chat: any, index: number) => {
            const realIndex = chats.indexOf(chat);
            return (
              <div
                key={chat.id ?? realIndex}
                data-tip={chat.title}
              ...
              >
                ...
                <div className="sb-item-right">
                  ...
                  <button
                    className={`sb-delete-btn${confirmDelete === realIndex ? " confirm" : ""}`}
                    onClick={(e) => handleDelete(e, realIndex)}
                    title={confirmDelete === realIndex ? "Click again to confirm" : "Delete session"}
                  >
                    {confirmDelete === realIndex ? (
                      <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
                        <path d="M2 3h8M4 3V2h4v1M5 5v4M7 5v4M3 3l.5 7h5L9 3" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    ) : (
        ...
        <div className="sb-footer">
          <div className="sb-footer-status">
            ...
            <div className="sb-footer-text">
              AI <strong>online</strong> · {chats.length} session{chats.length !== 1 ? "s" : ""} saved
            </div>
          </div>
        </div>
      </div>
